...
let entries=[], settings={cal:2000,water:2.5,weightGoal:null};
...
function hasAnyData(x){return !!x && ["weight","calories","water","exercise","notes","photo"].some(k=>x[k]!==null&&x[k]!==undefined&&x[k]!=="" )}
function load(){
...
function renderTargets(t){
  ...
  $("exerciseProgressText").textContent=t.exercise!=null?`${Math.round(t.exercise)} min`:"Not logged";
}
...
function renderReport(){
  ...
  const ac=avg("calories",d),aw=avg("water",d),ae=avg("exercise",d),awt=avg("weight",d);
  $("avgGrid").innerHTML=[
  ...
  if(ae!=null)tips.push(ae<30?`Exercise averaged ${Math.round(ae)} min/day. Consider whether a little more everyday movement fits your routine.`:`Exercise averaged ${Math.round(ae)} min/day this week.`);
...
function renderHistory(){
  ...
  $("historyList").innerHTML=a.length?a.map(x=>`<div class="history-item">
    ...
    <div class="history-values">${x.calories!=null?`${Math.round(x.calories)} kcal`:"—"} • ${x.water!=null?`${x.water.toFixed(1)} L`:"—"} • ${x.exercise!=null?`${Math.round(x.exercise)} min`:"—"}${x.photo?" • 📷 Photo":""}</div>
...
function clearAll(){
  ...
  entries=[];settings={cal:2000,water:2.5,weightGoal:null};
